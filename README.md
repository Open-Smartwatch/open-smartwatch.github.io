...
To build: use Visual Studio Code with the PlatformIO extension, you need to change the config.h.example file in ./include to suit your data, and change it to a header file (remove .example). Also, you need to select the right build flag in the platform.ini file, as well as upload port. Default is the GPS-version, for the light version please choose build_flags = -D DEBUG=1 . 
